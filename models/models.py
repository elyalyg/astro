...
from .attention import EyeAttention
...
def detrend(x, trend):
    """
    Divide the signal x by a lower-resolution trend. If x and trend
    have different time lengths, crop both to the shorter length.
    """
    # x, trend : (B, T₁, C) and (B, T₂, C)
    if x.shape[1] != trend.shape[1]:
        target_len = min(x.shape[1], trend.shape[1])
        warnings.warn(
            f"detrend: cropping x from length {x.shape[1]} and "
            f"trend from {trend.shape[1]} down to {target_len}"
        )
        x = x[:, :target_len, ...]
        trend = trend[:, :target_len, ...]
    return x / trend
